...
export function EditorBare(props: EditorProps & ComponentProps<"div">) {
	...
	function initEditor() {
		...
		editor.on("change", () => {
			...
			if (editorProps.form && editorProps.name)
				setValue(editorProps.form, editorProps.name, content);
		});

		createEffect(() => {
			if (themeStore.colorScheme && editor) {
				editor.setTheme(
					`ace/theme/${themeStore.colorScheme === "light" ? "kuroir" : "github_dark"}`,
				);
			}
		});
		...
		<div {...native} class={clsx("relative", native.class, native.classList)}>
			...
			<Show when={uploading()}>
				<Card
					class="absolute bottom-2 left-2 right-2"
					level="info"
					contentClass="z-50 px-4 p-2 flex items-center"
				>
			...
			<Show when={dragging()}>
				<Card
					class="absolute bottom-2 left-2 right-2 top-2"
					contentClass="z-50 flex items-center justify-center"
				>
...
export default function Editor(props: EditorProps & ComponentProps<"div">) {
	...
	return (
		<div
			{...nativeProps}
			class={clsx("flex flex-col space-y-1", nativeProps.class)}
		>
