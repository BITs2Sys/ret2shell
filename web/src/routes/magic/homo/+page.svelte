<script lang="ts">
  ...
  const homo = ((table: Record<number | string, string>) => {
    ...
    const getMinDiv = (num: number) => {
      for (let i = tableReversed.length; i >= 0; i--) if (num >= tableReversed[i]) return tableReversed[i]
      return 114514
    }
    const isDotRegex = /\.(\d+?)0{0,}$/
    const demolish = (num: number | string): string => {
      if (typeof num === 'string') num = parseInt(num)
      if (typeof num !== 'number') return `💩`

      if (num === Infinity || Number.isNaN(num)) return `💩`
      ...
      if (!Number.isInteger(num)) {
        ...
        // rescale
        const matched = num.toFixed(16).match(isDotRegex)
        if (matched) {
          let n = matched[1].length
          return `(${demolish(num * Math.pow(10, n))})/(10)^(${n})`
        }
      }

      if (table[num]) return String(num)
    ...
    return (num: number) => finisher(demolish(num))
  })({
...
<svelte:head><title>{$i18n.t('surprise.homo.title')} - {$platform.name}</title></svelte:head>
<div class="flex-1 flex flex-col items-center space-y-12 p-12">
  <img src={Homo} alt="压力马斯内.jpg" class="max-h-[24rem]" />
  <h1 class="text-3xl font-bold">{$i18n.t('surprise.homo.title')}</h1>
  <div class="flex flex-row space-x-4">
    <div>
      <RxInput class="flex-none" bind:value={input} icon="icon-[fluent--number-symbol-20-regular]" type="number" />
    </div>
    <p class="flex-1 p-3 text-base font-bold max-w-[48rem]">= {homo(input)}</p>
  </div>
</div>
