...
const ALTER_CHAR_TABLE: Lazy<HashMap<u8, Vec<u8>>> = Lazy::new(|| {
    ...
    map.insert(b'j', vec![b'J', b'7']);
    map.insert(b'k', vec![b'K', b'X']);
    map.insert(b'l', vec![b'L', b'1']);
    map.insert(b'm', vec![b'M', b'N']);
    map.insert(b'n', vec![b'N', b'M']);
    map.insert(b'o', vec![b'O', b'0']);
    map.insert(b'p', vec![b'P', b'9']);
    map.insert(b'q', vec![b'Q', b'9']);
    map.insert(b'r', vec![b'R', b'2']);
    map.insert(b's', vec![b'S', b'5']);
    map.insert(b't', vec![b'T', b'7']);
    map.insert(b'u', vec![b'U', b'v']);
    map.insert(b'v', vec![b'V', b'U']);
    map.insert(b'w', vec![b'W', b'V']);
...
fn get_flag_valid_part(flag: &str) -> String {
    ...
    for c in flag.to_ascii_lossy().bytes() {
...
fn get_flag_template_valid_part(flag_template: &str) -> String {
    ...
    for c in flag_template.to_ascii_lossy().bytes() {
...
pub fn get_user_id_from_flag(flag_template: &str, flag: &str) -> Result<i64, FlagError> {
    let flag_valid_part: Vec<u8> = get_flag_valid_part(flag).to_ascii_lossy().into_bytes();
