//! `SeaORM` Entity: AWD round scheduler state (one row per challenge).
...
pub async fn delete<C>(db: &C, challenge_id: i64) -> Result<(), DbErr>
where
  C: ConnectionTrait, {
  Entity::delete_by_id(challenge_id).exec(db).await.map(|_| ())
}
