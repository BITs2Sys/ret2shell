...
pub async fn spawn_game_workers(state: GlobalState) {
  ...
  // BITs2CTF fork (A6): fix worker lives in `worker/fix.rs`.
  tokio::spawn(super::fix::fix_worker(
    queue.clone(),
    database.clone(),
    state.cache.clone(),
    state.engine.clone(),
  ...
  // BITs2CTF fork: AWD + AWDP round workers.
  tokio::spawn(super::awd::spawn(state.clone()));
  tokio::spawn(super::awdp::spawn(state.clone()));
...
pub(super) fn get_award_rate(game: &game::Model, blood_state: i32) -> i32 {
...
async fn submission_worker_exec(
  ...
  // BITs2CTF fork: AWD attack submissions self-score (the attacker is credited via
  // `awd_steal`; there are no one-time challenge points), so handle them here and
  // return before the normal solve tail to avoid double-awarding.
  if let Some(awd_config) = challenge_bucket.awd().await.ok().flatten().filter(|c| c.enabled) {
    let content = submission.content.clone().unwrap_or_default();
    let (accepted, message) = match submission.team_id {
      Some(team_id) => {
        crate::utility::awd::verify_attack(&txn, &challenge, &awd_config, team_id, &content).await?
      }
      None => (false, "AWD attacks require a team".to_owned()),
    };
    let updated = submission::update(
      &txn,
      submission::Model {
        id: submission.id,
        solved: Some(accepted),
        result: Some(message),
        ..submission.clone()
      },
    )
  ...
  // stage 2: check the submission.
  // BITs2CTF fork: ISW challenges verify against the per-range injected flag
  // (hash lookup) rather than the rune checker — the flag is shared by the range's
  // team group and lives in `isw_flag`, not in a per-team checker computation.
  let isw_enabled = challenge_bucket
    .isw()
    .await
    .ok()
    .flatten()
    .is_some_and(|cfg| cfg.enabled);
  let (solved, result, audit) = if isw_enabled {
    let content = submission.content.clone().unwrap_or_default();
    let ok = crate::utility::isw::verify_submission(
      &txn,
      challenge.game_id,
      challenge.id,
      submission.team_id,
